...
describe('Phase 3 repository contracts', () => {
  ...
  it('keeps catalog, message, and session responsibilities separate', () => {
    ...
    const session: SessionRepository = { getCurrentUser: async () => null, subscribeToAuthState: () => () => undefined, signIn: async () => ({ user: null, requiresEmailConfirmation: false }), signUp: async () => ({ user: null, requiresEmailConfirmation: false }), signOut: async () => undefined, updateRoleForDemo: async (role) => ({ id: 'demo', displayName: 'Demo', role }) };
