...
const row = { id: 'm1', conversation_id: 'conversation-1', participant_id: 'barber-1', participant_name: 'Morgan', body: 'See you soon', created_at: '2026-08-25T10:00:00Z', unread: true };
...
describe('Supabase message repository', () => {
  it('maps thread rows and calls the discovery RPC', async () => {
    const mocked = client();
    await expect(createSupabaseMessageRepository(mocked.value).listThreads('user-1')).resolves.toEqual([{ id: 'm1', conversationId: 'conversation-1', participantId: 'barber-1', participantName: 'Morgan', body: 'See you soon', sentAt: row.created_at, unread: true }]);
  ...
  it('sends with the idempotency key and maps the returned message', async () => {
    ...
    await expect(createSupabaseMessageRepository(mocked.value).send('user-1', 'conversation-1', 'Hello', 'key-1')).resolves.toMatchObject({ conversationId: 'conversation-1', participantId: 'barber-1', body: 'See you soon', unread: false });
    expect(mocked.calls).toEqual([['send_message', { p_conversation_id: 'conversation-1', p_body: 'Hello', p_idempotency_key: 'key-1' }]]);
  });
  ...
  it('forwards distinct retry keys without inventing local deduplication', async () => {
    ...
    await repository.send('user-1', 'conversation-1', 'Hello', 'retry-1');
    await repository.send('user-1', 'conversation-1', 'Hello', 'retry-1');
    expect(mocked.calls).toEqual([
      ['send_message', { p_conversation_id: 'conversation-1', p_body: 'Hello', p_idempotency_key: 'retry-1' }],
      ['send_message', { p_conversation_id: 'conversation-1', p_body: 'Hello', p_idempotency_key: 'retry-1' }],
    ]);
  });
  ...
  it('marks a conversation read and propagates backend errors', async () => {
    const mocked = client([]);
    await expect(createSupabaseMessageRepository(mocked.value).markRead('user-1', 'conversation-1')).resolves.toBeUndefined();
    expect(mocked.calls).toEqual([['mark_conversation_read', { p_conversation_id: 'conversation-1' }]]);
