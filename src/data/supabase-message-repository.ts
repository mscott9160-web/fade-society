...
type MessageRow = { id: string; booking_id?: string | null; participant_id: string; participant_name: string; body: string; created_at: string; unread: boolean };
...
export function createSupabaseMessageRepository(client?: MessageClient | null): MessageRepository {
  ...
  const map = (row: MessageRow): Message => ({ id: row.id, bookingId: row.booking_id ?? undefined, participantId: row.participant_id, participantName: row.participant_name, body: row.body, sentAt: row.created_at, unread: row.unread });
  return {
