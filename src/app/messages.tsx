...
export default function MessagesScreen() {
  ...
  const { messages, bookings, messageLoading, messageError } = useAppStore();
  const theme = useCustomerTheme();
  const live = getDataMode() === 'supabase';
  const threads = Array.from(new Map(messages.map((message) => [`${message.bookingId ?? 'participant'}:${message.bookingId ?? message.participantId}`, message])).values());
  ...
  return <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]}><ScrollView contentContainerStyle={styles.container}>
    ...
    {live && messageLoading && <Text accessibilityRole="alert" style={[styles.subtitle, { color: theme.secondaryText }]}>Loading conversations...</Text>}
    {live && messageError && <Text accessibilityRole="alert" style={[styles.subtitle, { color: theme.accent }]}>Unable to load conversations. You may not be authorized to view these bookings.</Text>}
    {threads.map((message) => { const booking = bookings.find((item) => item.id === message.bookingId); return <Pressable key={`${message.bookingId ?? 'participant'}:${message.bookingId ?? message.participantId}`} accessibilityRole="button" accessibilityLabel={`Conversation with ${message.participantName}${booking ? ` about ${booking.serviceName}` : ''}${message.unread ? ', unread' : ''}`} accessibilityHint="Opens this conversation" onPress={() => router.push({ pathname: '/messages/[id]', params: { id: message.participantId, bookingId: message.bookingId } })} style={[styles.row, { backgroundColor: theme.surface, borderColor: theme.border }]}><View style={styles.avatar} /><View style={styles.copy}><View style={styles.top}><Text style={[styles.name, { color: theme.text, fontSize: 14 * theme.textScale }]}>{message.participantName}</Text>{message.unread && <Text accessibilityRole="text" style={[styles.unread, { color: theme.accent }]}>Unread</Text>}</View><Text style={[styles.context, { color: theme.text, fontSize: 12 * theme.textScale }]}>{booking ? `${booking.serviceName} · ${booking.studioName}` : 'Booking conversation'}</Text><Text style={[styles.preview, { color: theme.secondaryText, fontSize: 14 * theme.textScale }]}>{message.body || 'No messages yet. Start the conversation.'}</Text></View></Pressable>; })}
    {!messageLoading && threads.length === 0 && <View style={styles.empty}><Text style={styles.emptyTitle}>No booking conversations</Text><Text style={[styles.subtitle, { color: theme.secondaryText }]}>Conversations appear here when a booking is created.</Text></View>}
  </ScrollView></SafeAreaView>;
}

const styles = StyleSheet.create({ safeArea: { flex: 1, backgroundColor: '#F5F0EA' }, container: { padding: 18 }, title: { color: '#171717', fontSize: 32, fontWeight: '800', marginTop: 12 }, subtitle: { color: '#736C62', marginTop: 6, marginBottom: 18 }, supportCard: { backgroundColor: '#171717', borderRadius: 16, padding: 16, marginBottom: 16 }, supportTitle: { color: '#FFF', fontWeight: '800' }, supportText: { color: '#D9B778', marginTop: 5, fontWeight: '700' }, row: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF', borderRadius: 16, borderWidth: 1, borderColor: '#E9DED0', padding: 14, marginBottom: 10, minHeight: 72 }, avatar: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#DAB67A', marginRight: 12 }, copy: { flex: 1 }, top: { flexDirection: 'row', justifyContent: 'space-between' }, name: { color: '#171717', fontWeight: '800' }, context: { marginTop: 4, fontWeight: '700' }, unread: { color: '#8A6A3A', fontSize: 10, fontWeight: '800' }, preview: { color: '#736C62', marginTop: 5 }, empty: { backgroundColor: '#FFF', borderRadius: 16, padding: 18, marginTop: 12 }, emptyTitle: { color: '#171717', fontSize: 18, fontWeight: '800' } });
